...
export default function TeacherTimetable() {
  ...
  const loadTimetable = async () => {
    try {
      setLoading(true);
      const res = await fetchTimetable({ role: "teacher", teacher_id: user?.id });
      setTeacherTimetable(res.data.timetables);
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-50 p-6">
      ...
      <div className="mb-8 flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-foreground mb-2">My Timetable</h1>
      ...
      {teacherTimetable.length === 0 ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
          {days.map((day) => {
            const dayClasses = teacherTimetable.filter((item) => item.day === day);
            return (
                ...
                <div className="space-y-3">
                  {dayClasses.map((cls, idx) => (
                    ...
                    >
                      ...
                      <p className="text-xs font-medium text-blue-600 mb-2">{cls.class_name}</p>
